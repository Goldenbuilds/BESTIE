...
class VoiceAssistant:
        ...
        while True:
            with microphone as source:
                try:
                    ...
                    if "delete a file" in detected_phrase: #user needs to delete a file
                        self.activate_deletion()
        ...
        while True:
            with microphone as source:
                try:
                    ...
                    if "delete a file" in detected_phrase or "delete" in detected_phrase:
                        self.say("Okay I'll help you with file deletion")
                        self.gui.update_text("Okay I'll help with file deletion")
                        self.delete_file()
                    ...
                    if "go to sleep" in detected_phrase:
                        ...
                        self.gui.update_text("waiting for deletion command")
                        self.say("You need to tell me to delete a file for you")
                        
                except sr.RequestError:
                    self.gui.update_text("Failed to request results. Don't worry it is a problem with Google API")
                except sr.UnknownValueError:
                    pass #ignore if the speech couldn't be recognized
                        ...
                        if match:
                            ...
                            for file in os.listdir():
                                ...
                                if file_no_extn_cleaned == detected_phrase_no_extn:
                                    self.confirm_file_deletion(file)
                                    filename_found = file
                                    break
    ...
    def confirm_file_deletion(self, filename):
        while True:
            ...
            self.say("What file would you like to delete?")
            self.gui.update_text("What file would you like to delete ")
            new_filename = self.delete() #call delete again
            if not new_filename:
                break #Exit if no new file name provided
